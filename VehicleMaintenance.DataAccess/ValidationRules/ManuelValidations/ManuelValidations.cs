...
namespace VehicleMaintenance.DataAccess.ValidationRules.ManuelValidations
{
    public class ManuelValidations
    {
        ...
        public static ResponseDto VehicleValidation(VehicleDto request)
        {
            ...
            if (request.User.ID == 0) return Errors.UserIDRequired.JsonSerialize().JsonDeserialize<ResponseDto>();
            if (request.VehicleType.ID == 0) return Errors.VehicleTypeID.JsonSerialize().JsonDeserialize<ResponseDto>();
            if (request.User.ID == 0) return Errors.UserIDRequired.JsonSerialize().JsonDeserialize<ResponseDto>();
            return response;
        }
        ...
        public static ResponseDto MaintenanceValidation(MaintenanceDto request)
        {
            ...
            if (request.Vehicle.ID == 0) return Errors.VehicleIDRequired.JsonSerialize().JsonDeserialize<ResponseDto>();
            if (request.User.ID == 0) return Errors.UserIDRequired.JsonSerialize().JsonDeserialize<ResponseDto>();
            if (request.ResponsibleUser.ID == 0) return Errors.ResponsibleUserID.JsonSerialize().JsonDeserialize<ResponseDto>();
            if (request.Status.ID == 0) return Errors.StatusID.JsonSerialize().JsonDeserialize<ResponseDto>();
            if (request.PictureGroup.ID == 0) return Errors.PictureGroupID.JsonSerialize().JsonDeserialize<ResponseDto>();
        ...
        public static ResponseDto MaintenanceHistoryValidation(MaintenanceHistoryDto request)
        {
            ...
            if (request.ActionType.ID == 0) return Errors.ActionTypeIDRequired.JsonSerialize().JsonDeserialize<ResponseDto>();
            if (request.Maintenance.ID == 0) return Errors.MaintenanceIDRequired.JsonSerialize().JsonDeserialize<ResponseDto>();
